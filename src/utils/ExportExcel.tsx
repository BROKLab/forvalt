...
export const ExportExcel: React.FC<Props> = ({ ...props }) => {
    ...
    const exportToFile = (balances: BalanceAndMaybePrivateData[]) => {
        ...
        const ws = XLSX.utils.json_to_sheet(
            balances.map((bl) => {
                return {
                    navn: bl.name,
                    by: bl.city,
                    birtdate: bl.birthdate,
                    email: bl.email,
                    postcode: bl.postcode,
                    personummer: bl.identifier,
    ...
    return (
        <Box direction="row" alignSelf="end" gap="small" pad="small">
            ...
            />
            <Button primary size="small" label="EKSPORTER" onClick={(e) => exportToFile(props.data)} />
        </Box>
    );
};
