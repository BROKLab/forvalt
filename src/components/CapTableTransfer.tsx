...
export const CapTableTransfer: React.FC<Props> = ({ ...props }) => {
    ...
    const resolvePrivateTokenTransfer = async (privateUserData: PrivateTokenTransferData) => {
        ...
        let result;
        try {
            const results = (await signatureRequestHandler.results()) as { vp: string }[]
            result = results[0];
        } catch (error: any) {
        ...
        const res = await createUnclaimed(result.vp).catch((error: AxiosError<{ message: string; code: number }>) => {
